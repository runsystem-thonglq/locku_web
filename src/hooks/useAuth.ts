...
export const useAuth = () => {
  ...
  const login = useCallback(
    async (email: string, password: string) => {
      try {
        ...
        // backendAPI.logCredentials(email, password);
        const response: any = await backendAPI.login(email, password);
        console.log("response", response);
        // const responses = await authAPI.login(email, password);

        if (response?.user) {
          const now = new Date().getTime() + 3600 * 1000;
          const userData = { ...response?.user, timeExpires: now };
          setUser(userData);
          setMessage({
  ...
  const getAccountInfo = useCallback(
    async (idToken: string, refreshToken: string) => {
      try {
        setLoading(true);
  ...
  const updateAvatar = useCallback(
    async (profile_picture_url: string) => {
      if (!user?.idToken) return;
